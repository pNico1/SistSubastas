...
export default function PerfilScreen({ navigation }) {
  ...
  const totalPagado = pujas.reduce((acc, p) => acc + (parseFloat(p.importe) || 0), 0);
  const victorias = pujas.filter((p) => p.ganador === 'si').length;
      ...
      >
        ...
        <View style={styles.header}>
          ...
          {totalPagado > 0 && (
            <View style={{ marginTop: 14 }}>
              <Text style={styles.assetLabel}>VALOR TOTAL DE ACTIVOS</Text>
        ...
        <View style={styles.statsRow}>
          ...
          <StatCard
            icon="payments"
            label="Pagado"
            value={`$${(totalPagado / 1000).toFixed(1)}k`}
            accent={p.tertiary}
        ...
        <View style={styles.actionsCard}>
          <ActionRow
            icon="history"
            label="Historial de Adquisiciones"
            iconColor={p.primary}
