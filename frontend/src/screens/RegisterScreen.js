...
// Boton principal con gradiente kinetico
      ...
      >
        {loaded ? (
          ...
          <View style={styles.uploadIconWrap}>
            <Text style={styles.uploadIconEmoji}>{icon}</Text>
          </View>
        )}
...
function SecurityBadge() {
  return (
    <View style={styles.secBadge}>
      <Text style={styles.secBadgeText}>🔒 VERIFICACIÓN SEGURA</Text>
    </View>
  );
}
...
export default function RegisterScreen({ navigation }) {
  ...
  const paisItems = paises.map((pais) => ({ label: pais.nombre, value: pais.id }));
              ...
              <View>
                ...
                <View style={styles.uploadGrid}>
                  ...
                  <UploadCard
                    ...
                    sub="Tocá para subir o tomar foto"
                    icon="💳"
                    base64={form.fotoDorso}
                    onPress={() => pickFoto('dorso')}
                  />
                </View>
